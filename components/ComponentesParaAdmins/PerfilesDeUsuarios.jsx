...
const PerfilesDeUsuarios=({setArregloActualizar,item,i,arregloActualizar,seleccionar,setEditarInfoDelChofer,setEmailDelChoferEditar})=>{
    ...
    return(
        <View key={i} style={{alignItems:'center',flexDirection:'row',alignContent:'center'}}>
            ...
            >
                <Text style={{fontSize:16,fontWeight:'400',color:'#000'}}>{(item.tipoSubscripcion!='C' && acticando==false)?"Inactivo":(acticando && item.tipoSubscripcion!='C')?"Pendiente":"Activo"}</Text>
            </View>
        </View>
    )
}
