...
const PerfilesDeUsuarios=({item,i,arregloActualizar,seleccionar,setEditarInfoDelChofer,setEmailDelChoferEditar})=>{
    ...
    return(
        <View key={i} style={{alignItems:'center',flexDirection:'row',alignContent:'center'}}>
             {item.tipoSubscripcion!='B' && seleccionar==true && <View style={{borderWidth:2,borderColor:'red',height:25,width:25,alignItems:'center',marginRight:5}}
                        onTouchEnd={()=>{
            ...
            {(item.tipoSubscripcion=='B' || seleccionar==false) && <View style={{height:25,width:25,alignItems:'center',marginRight:5}}>
            ...
            <View style={[(item.tipoSubscripcion=='B')?{borderColor:'green'}:(acticando)?{borderColor:'yellow'}:{borderColor:'red'},{borderWidth:2},{width:'35%',alignItems:'center',borderRadius:6}]}>
                <Text style={{fontSize:16,fontWeight:'400',color:'#000'}}>{(item.tipoSubscripcion!='B' && acticando==false)?"Inactivo":(acticando)?"Pendiente":"Activo"}</Text>
            </View>
        </View>
    )
}
