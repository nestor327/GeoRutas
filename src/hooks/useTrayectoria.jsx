...
const useTrayectoria=(coordenadasOrigen,coordenadasDestino,setRutasTrayectoria,setVisualizarRutas,
    ...
    let url='http://georutas.us-east-2.elasticbeanstalk.com/api/CalculoDeMenorRuta/'+coordenadasOrigen.latitude+','+coordenadasOrigen.longitude+','+coordenadasDestino.latitude+','+coordenadasDestino.longitude+'?Email='+emailState+'&Token='+tokenState;
    ...
    let {data, error, isLoading,isError, isSuccess,status}=useQuery(["gets",coordenadasOrigen,coordenadasDestino,emailState,tokenState],async({queryKey})=>{
        return await fetch('http://georutas.us-east-2.elasticbeanstalk.com/api/CalculoDeMenorRuta/'+queryKey[1].longitude+','+queryKey[1].latitude+','+queryKey[2].longitude+','+queryKey[2].latitude+'?Email='+queryKey[3]+'&Token='+queryKey[4]).then(res=>datos=res.json()).catch(error => data=[]);
